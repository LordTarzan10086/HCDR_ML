...
def main() -> None:
    ...
    parser.add_argument("--control-mode", type=str, default="", help="cooperative | platform_only | arm_only")
    ...
    target_delta = np.array([args.target_dx, args.target_dy, args.target_dz], dtype=float)
    control_mode = str(payload["controller_cfg"].get("control_mode", "cooperative"))

    enable_viewer = bool(args.viewer or args.viewer_only)
    persist_viewer = bool(args.keep_viewer_open or args.viewer_only or args.reuse_viewer)
    ...
    if args.legacy_inprocess:
        ...
        try:
            ...
            if args.viewer_only:
                ...
                print("next run: add --viewer --reuse-viewer to the smoke command")
                return
            ...
            shutdown_routeb_services(
                ...
                shutdown_viewer=not persist_viewer,
                terminate_viewer=not persist_viewer,
            )
        service_mode = "ipc_headless_backend" + ("+viewer" if enable_viewer else "")
        if bool(args.reuse_viewer):
            service_mode += "+reused"
        elif bool(args.keep_viewer_open):
            service_mode += "+kept"
    last = logs[-1]
    ...
    print(f"target delta [m]: {target_delta}")
    ...
    print(f"min platform limit margin: {metrics['min_platform_limit_margin']}")
...
def summarize_logs(logs: list[dict], target_world: np.ndarray) -> dict:
    ...
    for entry in logs:
        ...
        if first_fail_step is None and str(solver_diag.get("fail_reason", "none")) != "none":
            first_fail_step = int(entry["step"]) + 1
        if str(solver_diag.get("fail_reason", "none")) != "none":
            solver_fail_count += 1
        if bool(entry.get("diagnostics", {}).get("hold_active", False)):
            hold_count += 1
        ...
        if "cable_forces" in snapshot:
            max_cable_force = max(max_cable_force, float(np.max(np.asarray(snapshot["cable_forces"], dtype=float))))
...
def export_online_smoke_results(
    ...
    summary_payload = {
        ...
        "target_delta": [float(args.target_dx), float(args.target_dy), float(args.target_dz)],
