...
public:
    ...
    void insert(
        ...
        double psi_rad,
        double gamma_n) {
        insert_quantized(quantize_point(point_m, voxel_size_m_), psi_rad, gamma_n);
    }
    ...
    void insert_quantized(
        ...
        if (iter == records_.end()) {
            ...
            record.best_psi_rad = psi_rad;
            record.best_gamma_n = gamma_n;
        ...
        if (std::isfinite(gamma_n) &&
            (!std::isfinite(iter->second.best_gamma_n) || gamma_n > iter->second.best_gamma_n)) {
            iter->second.best_gamma_n = gamma_n;
            iter->second.best_psi_rad = psi_rad;
        }
    }
    ...
    void merge_from(const VoxelGridPointCloud& other) {
        ...
        for (const auto& [key, value] : other.records_) {
            insert_quantized(key, value.best_psi_rad, value.best_gamma_n, value.hit_count);
        }
    }
