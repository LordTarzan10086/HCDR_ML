...
PlatformGeometry::PlatformGeometry(const WorkspaceStaticConfig& config) : config_(config) {
    screw_positions_xy_m_ <<
        config_.frame_half_length_m, -config_.frame_half_length_m, -config_.frame_half_length_m, config_.frame_half_length_m,
        config_.frame_half_length_m, config_.frame_half_length_m, -config_.frame_half_length_m, -config_.frame_half_length_m;
...
PlatformPoseData PlatformGeometry::evaluate_pose(double x_m, double y_m, double psi_rad) const {
    ...
    const Eigen::Matrix<double, 3, kCableCount> attach_world_m =
        output.platform_center_world_m.replicate(1, kCableCount) + output.lever_arms_world_m;
    const Eigen::Matrix<double, 3, kCableCount> cable_vectors_world_m =
        cable_anchors_world_m_ - attach_world_m;

    for (int cable_index = 0; cable_index < kCableCount; ++cable_index) {
        const double full_length_m = cable_vectors_world_m.col(cable_index).norm();
        const Eigen::Vector2d cable_xy = cable_vectors_world_m.block<2, 1>(0, cable_index);
        const double planar_length_m = cable_xy.norm();
        output.full_length_m(cable_index) = full_length_m;
        output.planar_length_m(cable_index) = planar_length_m;
        if (full_length_m <= config_.eps_rank || planar_length_m <= config_.eps_rank) {
            output.rank = 0;
            output.sigma_min = 0.0;
            output.legacy_a2d.setZero();
            output.a2d.setZero();
            return output;
        }

        const double unit_x = cable_xy.x() / planar_length_m;
        const double unit_y = cable_xy.y() / planar_length_m;
        const double legacy_torque_z =
            output.lever_arms_world_m(0, cable_index) * unit_y -
            output.lever_arms_world_m(1, cable_index) * unit_x;
        const double projection_coeff = planar_length_m / full_length_m;

        output.planar_projection_coeff(cable_index) = projection_coeff;
        output.legacy_a2d(0, cable_index) = unit_x;
        output.legacy_a2d(1, cable_index) = unit_y;
        output.legacy_a2d(2, cable_index) = legacy_torque_z;

        output.a2d(0, cable_index) = projection_coeff * unit_x;
        output.a2d(1, cable_index) = projection_coeff * unit_y;
        output.a2d(2, cable_index) = projection_coeff * legacy_torque_z;
    }
    ...
    output.rank = static_cast<int>((singular_values.array() > config_.eps_rank).count());
    return output;
}
