...
fs::path resolve_workspace_relative_path(
...
WorkspaceStaticConfig load_workspace_static_config(const fs::path& json_path) {
    ...
    config.enable_mode1 = get_bool(json_text, "enable_mode1", config.enable_mode1);
    config.enable_mode2 = get_bool(json_text, "enable_mode2", config.enable_mode2);
    config.enable_mode3 = get_bool(json_text, "enable_mode3", config.enable_mode3);
    config.voxel_size_m = get_number(json_text, "voxel_size", config.voxel_size_m);
    config.mode1_xy_resolution = static_cast<int>(
        std::llround(get_number(json_text, "mode1_xy_resolution", config.mode1_xy_resolution)));
    config.mode2_arm_samples = static_cast<int>(
        std::llround(get_number(json_text, "mode2_arm_samples", config.mode2_arm_samples)));
    config.mode3_xy_resolution = static_cast<int>(
        std::llround(get_number(json_text, "mode3_xy_resolution", config.mode3_xy_resolution)));
    config.psi_samples = static_cast<int>(
        std::llround(get_number(json_text, "psi_samples", config.psi_samples)));
    ...
    config.platform_half_thickness_m = get_number(
        json_text, "platform_half_thickness", config.platform_half_thickness_m);
    config.pulley_spacing_m = get_number(json_text, "pulley_spacing", config.pulley_spacing_m);
    config.platform_xy_half_span_m = get_number(
        json_text,
        "platform_xy_half_span",
        config.frame_half_length_m - config.platform_half_side_m);
    config.output_union = get_bool(json_text, "output_union", config.output_union);
    config.output_fixedpsi_diagnostics = get_bool(
        json_text,
        "output_fixedpsi_diagnostics",
        config.output_fixedpsi_diagnostics);
    ...
    if (!platform_init.empty()) {
        config.platform_init.x_m = get_number(platform_init, "x", 0.0);
        config.platform_init.y_m = get_number(platform_init, "y", 0.0);
        config.platform_init.psi_rad = deg_to_rad(get_number(platform_init, "psi_deg", 0.0));
    }
    ...
    config.urdf_path = resolve_workspace_relative_path(
        ...
        get_string(json_text, "urdf_path", "../kortex_description/robots/gen3_lite_gen3_lite_2f_local.urdf"));
    ...
    if (config.mode1_xy_resolution < 2 || config.mode3_xy_resolution < 2 || config.psi_samples < 1) {
        throw std::runtime_error("Resolution fields must be positive and psi_samples must be >= 1.");
    }
    ...
    if (!config.enable_mode1 && !config.enable_mode2 && !config.enable_mode3) {
        throw std::runtime_error("At least one of enable_mode1/enable_mode2/enable_mode3 must be true.");
    }
